...
class DroneCategoryTests(APITestCase):
    def post_drone_category(self, name):
        """This method builds the URL and the data dictionary to compose and send an HTTP POST request"""
    ...
    def test_post_and_get_drone_category(self):
        """Test if we can create a new DroneCategory and then retrieve it"""
    ...
    def test_post_existing_drone_category_name(self):
        """Test the unique constraint for the drone category name"""
        url = reverse(views.DroneCategoryList.name)
    ...
    def test_filter_drone_category_by_name(self):
        """Test if we can filter a drone category by name"""
    ...
    def tests_get_drone_categories_collection(self):
        """Test if we can retrieve the drone categories collection"""
    ...
    def test_update_drone_category(self):
        """Test if we can update a single field for a drone category"""
        ...
        path_response = self.client.patch(url, data, format='json')
        assert path_response.status_code == status.HTTP_200_OK
        assert path_response.data['name'] == updated_drone_category_name

    def test_get_drone_category(self):
        """Test if  we can retrieve a single category with an HTTP GET request"""
...
class PilotTests(APITestCase):
    ...
    def create_user_and_set_token_credentials(self):
        user = User.objects.create_user('anotheruser', 'anotheruser@example.com', 'jdkfjrnamsi')
        token = Token.objects.create(user=user)
        self.client.credentials(
            HTTP_AUTHORIZATION='Token {0}'.format(token.key))

    def test_post_and_get_pilot(self):
        """Test if we can create a new Pilot and then retrieve it and cannot retrieve the persisted
        pilot without a token
        """
    ...
    def test_try_to_post_pilot_without_token(self):
        """Test if we cannot create a pilot without a token"""
